...
var client = src_default(process.env.DB_URL);
...
__export(Schema_exports, {
  workDB: () => workDB,
  workStatusDB: () => workStatusDB
});
import { pgTable, text, timestamp, pgEnum } from "drizzle-orm/pg-core";
var workStatusDB = pgEnum("work_status", [
  "todo",
  "in-progress",
  "done"
]);
var workDB = pgTable("workdb", {
...
app.get("/", async (c) => {
  const works = await db.select().from(Schema_exports.workDB);
  return c.json(works);
});
app.post("/add", async (c) => {
  ...
  await db.insert(Schema_exports.workDB).values(work);
  return c.json(work, 201);
});
app.patch("/update/:id", async (c) => {
  const id = c.req.param("id");
  const updates = await c.req.json();
  const work = await db.update(Schema_exports.workDB).set(updates).where(eq(Schema_exports.workDB.id, id));
  if (!work) return c.json({ message: "Not found" }, 404);
  return c.json(work);
});
app.delete("/delete/:id", async (c) => {
  const id = c.req.param("id");
  await db.delete(Schema_exports.workDB).where(eq(Schema_exports.workDB.id, id));
  return c.body(null, 204);
});
var GET = handle(app);
var POST = handle(app);
